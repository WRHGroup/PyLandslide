...
def test_json_loading():
    ...
    features_file_path = os.path.join(os.getcwd(),'tests/files/ml_data/features.csv')
    assert(WeightRangeModel.features_file == features_file_path)
    targets_file_path = os.path.join(os.getcwd(),'tests/files/ml_data/targets.csv')
...
def test_overall_accuracy_metric_model():
    ...
    importances = WeightRangeModel.feature_importance_model(targets=Y_train[target_names[0]], features=X_train, max_tree_depth=WeightRangeModel.max_tree_depth, n_estimators=WeightRangeModel.number_trees, cores=WeightRangeModel.cores)
